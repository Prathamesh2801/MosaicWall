...
export default function MosaicWallM({
  imageURL,
  onReset,
  onUploadAgain,
  tiles: propTiles,
      ...
      <div className="max-w-7xl mx-auto">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            ...
            {/* New: Upload Again (preserve tiles) */}
            <motion.button
              initial={{ opacity: 0, scale: 0.8 }}
              animate={{ opacity: 1, scale: 1 }}
              whileHover={{ scale: 1.05 }}
              whileTap={{ scale: 0.95 }}
              onClick={() => {
                if (onUploadAgain) onUploadAgain();
              }}
              className="flex items-center gap-3 bg-gradient-to-r from-green-600 to-emerald-500 text-white font-semibold py-3 px-6 rounded-xl shadow-lg transition-all"
            >
              <span>Upload Again (keep revealed tiles)</span>
            </motion.button>
          </div>
